...
gint Buttons (GtkWidget *fixed)
{
   ...
   gtk_fixed_put(GTK_FIXED(fixed),button,400,150);
      ...
      char *argv[])
{
  ...
  g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(callback),"Undo button pressed");

  button = gtk_button_new_with_label("Hint");
  g_signal_connect(G_OBJECT(button),"clicked",G_CALLBACK(callback),"Hint button pressed");
