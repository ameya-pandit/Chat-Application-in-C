...
TNODE *checkmove(GAME *game){
    char postion[2];
    int spot=-1;
    char piece;
    bool flag=false;
    TNODE *allmoves=CreateTree();
    for(int i=0;i<64;i++){
        do{     
            spot++;
            piece=game->currentBoard.piece[spot];
        ...
        if((piece=='K')||(piece=='k')){
            kingmoves(game->currentBoard, spot, allmoves);
            i+=2;
        }
        else if((piece=='Q')||(piece=='q')){
            queenmoves(game->currentBoard, spot, allmoves);
            i+=2;
        }
        else if((piece=='R')||(piece=='r')){
            rookmoves(game->currentBoard, spot, allmoves);
            i+=2;
        }
        else if((piece=='B')||(piece=='b')){
            bishopmoves(game->currentBoard, spot, allmoves);
            i+=2;
        }
        else if((piece=='N')||(piece=='n')){
            knightmoves(game->currentBoard, spot, allmoves);
            i+=2;
        }
        else if((piece=='P')||(piece=='p')){
            pawnmoves(game->currentBoard, spot, allmoves);
            i+=2;
        }
    }
    return allmoves;
...
bool check(GBOARD currentBoard){
    int king=-1;
    bool pawn=false, queen=false, knight=false, bishop=false, rook=false,
    //finding the king
    //Player 1 king
    ...
    if(queen==true || pawn==true || rook==true || bishop==true || knight==true){
        return true;
    }
    else{
        return false;
...
int checkmate(GBOARD currentBoard){
    ...
    if(check(currentBoard)==true){
        if (currentBoard.playerflag==1){
            ...
            MLIST *kingcheck=kingmoves(currentBoard, king); 
            //checking if all possible moves for the king are in check still
            if(kingcheck->first==NULL){
                return 1; 
            }           
        }
        else{
